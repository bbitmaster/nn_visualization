...
if(len(sys.argv) > 1):
    ...
    params_file = 'visualize_cluster_params.py'
...
#todo: work on initialization techniques
net.layer[0].centroids = ((np.random.random((net.layer[0].weights.shape)) - 0.5)*2.0)
net.layer[0].select_func = cluster_select_func
...
while(epoch < p['total_epochs']):
    ...
    if epoch < p['training_epochs']:
        net.back_propagate()
        net.update_weights();
        cluster_update_func(net.layer[0])
